...
export async function createApp() {
  ...
  const app = await NestFactory.create<NestExpressApplication>(AppModule, {
    ...
    rawBody: true,
    bodyParser: true,
    logger: slax.slax.stable ? ['log'] : ['verbose'],
  });
  ...
  if (slax.isSelfhosted && slax.telemetry.enabled) {
    const mixpanel = await import('mixpanel');
    mixpanel.init(slax.telemetry.token).track('selfhost-server-started', {
      version: slax.version,
    });
  }
