...
export class AppModuleBuilder {
  private readonly modules: slaxModule[] = [];
  constructor(private readonly config: Config) {}

  use(...modules: slaxModule[]): this {
    modules.forEach(m => {
      ...
      if (requirements?.length) {
        ...
        if (nonMetRequirements.length) {
          ...
          new Logger(name).warn(
            ...
            ...nonMetRequirements.map(config => `  slax.${config}`)
          );
          return;
        }
      }
...
function buildAppModule() {
  const factor = new AppModuleBuilder(slax);
  ...
  // plugin modules
  slax.plugins.enabled.forEach(name => {
