# Building slax Desktop Client App
...
Before you start building slax Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
```shell
BUILD_TYPE=canary SKIP_NX_CACHE=1 yarn workspace @slax/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn workspace @slax/electron make
```
...
yarn workspace @slax/electron package
yarn workspace @slax/electron make-squirrel
yarn workspace @slax/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <slax-repo>/packages/frontend/electron/out/make
```
