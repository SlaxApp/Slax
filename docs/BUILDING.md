...
slax client has both **Node.js** & **Rust** toolchains.
...
```sh
git clone https://github.com/slaxapp/slax
```
...
Note: use `strip` from system instead of `binutils` if you are running MacOS. [see problem here](https://github.com/slaxapp/slax/discussions/2840)
...
Also make sure you have built the `@slax/core` workspace before running E2E tests.
...
```shell
# there are `slax-local`, `slax-migration`, `slax-local`, `slax-prototype` e2e tests,
